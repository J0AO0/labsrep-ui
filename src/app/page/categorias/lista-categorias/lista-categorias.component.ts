...
export class ListaCategoriasComponent implements OnInit, AfterViewInit {
  ...
  onClear() {
    ...
    this.filtro = new FiltroCategorias();
    this.filtroDefault();
  ...
  ngOnInit() {
    this.title.setTitle('Lista de Categorias');
    this.filtroDefault();
    this.items = [
      {
        label: 'Ativo / Inativo',
    ...
    if (!localStorage.getItem('categoriasColumns')) {
      this.setColumnsDefaultValue();
    } else {
      // get selected columns from local storage
      this.selectedColumns = JSON.parse(localStorage.getItem('categoriasColumns'));
    }
  }
